    ...
    <Navbars />  
      ...
      <table className="doctable">
        ...
        <tbody>
          {posts.map(post=>
            <tr key={post.id}>
              <td scope='row' className='docrow'>{post.startDay}~{post.endDay}</td>
              <td className='docrow'>
                <Link to={`/detail/${post.id}`} key={post.id}>
                  {post.title}
                </Link>
              </td>
              <td className='docrow'>{post.writer}</td>
              <td className='docrow'>{post.created_at.substring(10, 0)}</td>
            </tr>
          )}
        </tbody>
      </table>
